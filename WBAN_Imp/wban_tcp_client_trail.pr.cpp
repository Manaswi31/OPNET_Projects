/* Process model C++ form file: wban_tcp_client_trail.pr.cpp */
...
const char wban_tcp_client_trail_pr_cpp [] = "MIL_3_Tfile_Hdr_ 1800 30A modeler 7 57228CE4 57228CE4 1 volta chaganti 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 468d 2                                                                                                                                                                                                                                                                                                                                                                                                           ";
#include <string.h>
...
class wban_tcp_client_trail_state
	{
	private:
	...
	public:
		wban_tcp_client_trail_state (void);
...
#define ici_ptr                 		pr_state_ptr->ici_ptr
...
client_active_transport_conn_open (void* PRG_ARG_UNUSED (state_ptr), int PRG_ARG_UNUSED (code))
	...
	//tcp_conn_open_options_local_port_set (&conn_options, 5800);
	conn_options.local_address = INETC_ADDRESS_INVALID;
	conn_options.local_port = loc_port;
	conn_options.ip_tos = 0;
	conn_options.inet_support = OPC_TRUE;
...
ip_addr_family_params_objid_get (Objid parent_objid)
	...
	for (row_index = 0; row_index < num_addr_families; row_index++)
		...
		op_ima_obj_attr_get_int32 (af_params_row_id, "Loc Port", &ip_addr[row_index].local_port);
		printf("REmote IP Address : %s ---- Remort Port : %d ---- Local Port : %d\n", ip_addr[row_index].rem_ip_addr, ip_addr[row_index].rem_port, ip_addr[row_index].local_port);  
...
extern "C"
{
	...
	void wban_tcp_client_trail (OP_SIM_CONTEXT_ARG_OPT)
		{
		((wban_tcp_client_trail_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->wban_tcp_client_trail (OP_SIM_CONTEXT_PTR_OPT);
		}
	...
	void _op_wban_tcp_client_trail_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((wban_tcp_client_trail_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_wban_tcp_client_trail_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_wban_tcp_client_trail_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (wban_tcp_client_trail_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
wban_tcp_client_trail_state::wban_tcp_client_trail (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (wban_tcp_client_trail_state::wban_tcp_client_trail ());
	try
		{
		/* Temporary Variables */
		//Packet*			pkptr;
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "INIT", "wban_tcp_client_trail [INIT enter execs]")
				...
				loc_port = ip_addr[count].local_port;
				printf("My IP address : %s *** REmote IP Address : %s **** Remort Port : %d **** Local Port : %d\n", ip_addr_str, ip_addr[count].rem_ip_addr, ip_addr[count].rem_port, ip_addr[count].local_port);  
			...
			FSM_STATE_EXIT_UNFORCED (5, "idle", "wban_tcp_client_trail [idle exit execs]")
				...
				op_ici_print(status_iciptr);
					...
					/*	Generate trace messsage							*/
					if ((status_ind == TCPC_IND_CLOSED|| status_ind == TCPC_IND_ABORTED) && op_prg_odb_ltrace_active ("tcp"))
						{
							...
							printf("Local key : %d \n", local_key);
						op_prg_odb_print_major ("Received ABORT indication in OPEN state, proceeding to OPEN IND again state.", OPC_NIL);
						}
				}
				}
			...
			FSM_STATE_EXIT_FORCED (6, "ESTAB", "wban_tcp_client_trail [ESTAB exit execs]")
				FSM_PROFILE_SECTION_IN ("wban_tcp_client_trail [ESTAB exit execs]", state6_exit_exec)
				{
					...
					for(int i = 0 ; i < 3; i++)
						{
							...
							if (bgp_conn_info_ptr == OPC_NIL)
								...
								inet_address_print(loc_addr_str, loc_addr);
								...
								//if(strcmp(server_addr_str, "192.0.0.14") == 0){
										op_ici_attr_set (transport_ici_ptr, "conn_id", 1); 
									//	if((strcmp(loc_addr_str, "192.0.0.6") == 0) || (strcmp(loc_addr_str, "192.0.0.2") == 0))
											//op_ici_attr_set (transport_ici_ptr, "local_key", 16777216);
									//	else
											op_ici_attr_set (transport_ici_ptr, "local_key", 0);
											//op_ici_attr_set (transport_ici_ptr, "conn_id", 2);
											op_ici_attr_set (transport_ici_ptr, "local_port", ip_addr[i].local_port);
								 ...
								 // else if (strcmp(server_addr_str, "192.0.0.25") == 0){
									   op_ici_attr_set (transport_ici_ptr, "conn_id", 2); 
								 //op_ici_attr_set (transport_ici_ptr, "conn_id", 3); 
								 	...
								 	//	 else if (strcmp(server_addr_str, "192.0.0.29") == 0){
									   op_ici_attr_set (transport_ici_ptr, "conn_id", 3); 
									   //op_ici_attr_set (transport_ici_ptr, "conn_id", 4); 
									...
									op_ici_attr_set (transport_ici_ptr, "local_key", local_key);
											...
											printf("Loc addr : %s \t\t rem addr : %s\n remote port %d \t\t local port : %d\n local_key : %d", ip_addr_str, server_addr_str, ip_addr[i].rem_port, ip_addr[i].local_port, local_key);
				...
				else
				{
					...
					else
				    {
								...
								rem_addr = inet_address_from_ipv4_address_create(ip_address_create(dest_address_str));//inet_address_copy(remote_inet_ip_addr);// ;
										...
										//if(strcmp(dest_address_str, "192.0.0.14") == 0){
											//op_ici_attr_set (transport_ici_ptr, "conn_id", tcp_conn_id); 
											op_ici_attr_set (transport_ici_ptr, "local_key", 0);
																		op_ici_attr_set (transport_ici_ptr, "conn_id", 1);
										...
										//else
											//op_ici_attr_set (transport_ici_ptr, "local_key", 0);
																		//op_ici_attr_set (transport_ici_ptr, "conn_id", 2);
											op_ici_attr_set (transport_ici_ptr, "local_port", ip_addr[Index].local_port);
											op_ici_attr_set (transport_ici_ptr, "rem_addr", inet_address_copy_dynamic (&rem_addr));
											op_ici_attr_set (transport_ici_ptr, "rem_port", ip_addr[Index].rem_port);
											op_ici_attr_set (transport_ici_ptr, "local_addr", inet_address_copy_dynamic (&loc_addr));
											}
								   else if (strcmp(dest_address_str, "192.0.0.30") == 0){
								   // else if (strcmp(dest_address_str, "192.0.0.25") == 0){
									   op_ici_attr_set (transport_ici_ptr, "conn_id", 2); 
									  op_ici_attr_set (transport_ici_ptr, "local_key", 1);
									   //	op_ici_attr_set (transport_ici_ptr, "conn_id", 3);
									   Index = 1;
								 ...
								 //else if (strcmp(dest_address_str, "192.0.0.29") == 0){
									   op_ici_attr_set (transport_ici_ptr, "conn_id", 3);
									   Index = 2;
									  op_ici_attr_set (transport_ici_ptr, "local_key", 2);
									   						//	op_ici_attr_set (transport_ici_ptr, "conn_id", 4);
									...
									//Index = 0;
										...
										//	op_ici_attr_set (transport_ici_ptr, "rem_port", ip_addr[i].rem_port);
											...
											op_ici_print(transport_ici_ptr);
											printf("Loc addr : %s \t\t rem addr : %s\n remote port %d \t\t local port : %d\n local_key : %d", ip_addr_str, server_addr_str, ip_addr[Index].rem_port, ip_addr[Index].local_port, local_key);
		...
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (wban_tcp_client_trail)",
...
void
wban_tcp_client_trail_state::_op_wban_tcp_client_trail_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}

void
wban_tcp_client_trail_state::operator delete (void* ptr)
	{
	FIN (wban_tcp_client_trail_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

wban_tcp_client_trail_state::~wban_tcp_client_trail_state (void)
	{

	FIN (wban_tcp_client_trail_state::~wban_tcp_client_trail_state ())
...
void *
wban_tcp_client_trail_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (wban_tcp_client_trail_state::operator new ());

	new_ptr = Vos_Alloc_Object (wban_tcp_client_trail_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
wban_tcp_client_trail_state::wban_tcp_client_trail_state (void) :
		_op_current_block (0)
	{
		...
		_op_current_state = "wban_tcp_client_trail [INIT enter execs]";
#endif
	}
...
_op_wban_tcp_client_trail_init (int * init_block_ptr)
	...
	wban_tcp_client_trail_state::obtype = Vos_Define_Object_Prstate ("proc state vars (wban_tcp_client_trail)",
		sizeof (wban_tcp_client_trail_state));
	*init_block_ptr = 0;

	FRET (wban_tcp_client_trail_state::obtype)
	}

VosT_Address
_op_wban_tcp_client_trail_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	wban_tcp_client_trail_state * ptr;
	FIN_MT (_op_wban_tcp_client_trail_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new wban_tcp_client_trail_state;
...
_op_wban_tcp_client_trail_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	...
	if (strcmp ("loc_port" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->loc_port);
		FOUT
		}
