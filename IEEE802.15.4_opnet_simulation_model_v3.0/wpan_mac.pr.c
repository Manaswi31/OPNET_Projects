/* Process model C form file: wpan_mac.pr.c */
...
const char wpan_mac_pr_c [] = "MIL_3_Tfile_Hdr_ 1800 30A op_runsim_opt 7 56D7210E 56D7210E 1 volta chaganti 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 468d 2                                                                                                                                                                                                                                                                                                                                                                                                     ";
#include <string.h>
...
#define pr_state_ptr            		((wpan_mac_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))
...
static void wpan_mac_init() {
	...
	printf("Node attributess pan id : %d\n", node_attributes.pan_id);
...
static void wpan_parse_incoming_frame() {
	...
	switch (intrpt_strm) {
		case STRM_FROM_NWK_TO_MAC: /* incomming packet's paylod (MSDU) from Network layer */
		{
			...
			printf("Received packet from NETWORK LAYER : %d DEst address \n", dest_address);
		...
		case STRM_FROM_RADIO_TO_MAC: /* PHY packet (PPDU) from radio receiver */
		{
			...
			if (frame_type_fd == BEACON_FRAME_TYPE) { /* I have received beacon packet */
				...
				//printf("My parent address : %d \t\t source address : %d \t\t  pan id : %d \t\t node_attributes.pan_id : %d \n\n", my_parent_address,source_address, pan_id,node_attributes.pan_id);  
...
static void wpan_encapsulate_and_enqueue_command_frame (Packet* msdu, Boolean ack, int dest_address) {
	...
	if (op_subq_pk_insert(SUBQ_CAP, mac_frame, OPC_QPOS_TAIL) == OPC_QINS_OK) {
		...
		if (enable_log) {
			...
			printf (" [%s (#%d)] t=%f  -> COMMAND packet cannot be enqueuing - PACKET IS DROPPED !!!! \n\n", device_name, my_address, op_sim_time());
		}
...
void
wpan_mac (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
...
void
_op_wpan_mac_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_wpan_mac_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_wpan_mac_terminate ())
...
/* local variable prs_ptr in _op_wpan_mac_svar function. */
...
_op_wpan_mac_init (int * init_block_ptr)
	...
	obtype = Vos_Define_Object_Prstate ("proc state vars (wpan_mac)",
...
VosT_Address
_op_wpan_mac_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	wpan_mac_state * ptr;
	FIN_MT (_op_wpan_mac_alloc (obtype))

	ptr = (wpan_mac_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "wpan_mac [init enter execs]";
#endif
		}
