/* Process model C form file: wpan_application.pr.c */
/* Portions of this file copyright 2014 by Riverbed Technology. All rights reserved. */


/*
 =========================== NOTE ==========================
 This file is automatically generated from wpan_application.pr.m
...
const char wpan_application_pr_c [] = "MIL_3_Tfile_Hdr_ 1800 30A modeler 7 5511DFA8 5511DFA8 1 rhel04 chaganti 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 468d 2                                                                                                                                                                                                                                                                                                                                                                                                          ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	wpan_application_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((wpan_application_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void wpan_application (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_wpan_application_init (int * init_block_ptr);
	void _op_wpan_application_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_wpan_application_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_wpan_application_alloc (VosT_Obtype, int);
	void _op_wpan_application_svar (void *, const char *, void **);
...
void
wpan_application (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (wpan_application ());

		{


		FSM_ENTER ("wpan_application")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "wpan_application [init enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_application [init enter execs]", state0_enter_exec)
				{
				/* Initialization */
				wpan_appl_init ();
				}
			...
			FSM_STATE_EXIT_FORCED (0, "init", "wpan_application [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_1", "wpan_application [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "wpan_application [idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"wpan_application")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "wpan_application [idle exit execs]")
				FSM_PROFILE_SECTION_IN ("wpan_application [idle exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("wpan_application [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "SEND_BE", "", "idle", "send BE", "send best-effort packet", "wpan_application [idle -> send BE : SEND_BE / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "SEND_RT", "", "idle", "send RT", "send real-time packet", "wpan_application [idle -> send RT : SEND_RT / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "RECEIVE_PCK", "", "idle", "receive pck", "receive packet from NWK", "wpan_application [idle -> receive pck : RECEIVE_PCK / ]")
				FSM_CASE_TRANSIT (3, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_13", "wpan_application [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "send BE", state2_enter_exec, "wpan_application [send BE enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_application [send BE enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "send BE", "wpan_application [send BE exit execs]")


			/** state (send BE) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send BE", "idle", "tr_6", "wpan_application [send BE -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send RT) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send RT", state3_enter_exec, "wpan_application [send RT enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_application [send RT enter execs]", state3_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "send RT", "wpan_application [send RT exit execs]")


			/** state (send RT) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send RT", "idle", "tr_8", "wpan_application [send RT -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (receive pck) enter executives **/
			FSM_STATE_ENTER_FORCED (4, "receive pck", state4_enter_exec, "wpan_application [receive pck enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_application [receive pck enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (4, "receive pck", "wpan_application [receive pck exit execs]")


			/** state (receive pck) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "receive pck", "idle", "tr_10", "wpan_application [receive pck -> idle : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_wpan_application_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_wpan_application_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_wpan_application_terminate ())
...
/* local variable prs_ptr in _op_wpan_application_svar function. */
#undef self_id
#undef parent_id
#undef device_mode
#undef device_name
...
VosT_Obtype
_op_wpan_application_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_wpan_application_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (wpan_application)",
		sizeof (wpan_application_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_wpan_application_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	wpan_application_state * ptr;
	FIN_MT (_op_wpan_application_alloc (obtype))

	ptr = (wpan_application_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "wpan_application [init enter execs]";
#endif
		}
...
void
_op_wpan_application_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	wpan_application_state		*prs_ptr;

	FIN_MT (_op_wpan_application_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (wpan_application_state *)gen_ptr;
