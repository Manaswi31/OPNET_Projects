/* Process model C form file: wpan_analyzer.pr.c */
/* Portions of this file copyright 2014 by Riverbed Technology. All rights reserved. */


/*
 =========================== NOTE ==========================
 This file is automatically generated from wpan_analyzer.pr.m
...
const char wpan_analyzer_pr_c [] = "MIL_3_Tfile_Hdr_ 1800 30A modeler 7 566C7AD6 566C7AD6 1 volta chaganti 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 468d 2                                                                                                                                                                                                                                                                                                                                                                                                           ";
#include <string.h>
...
#define pr_state_ptr            		((wpan_analyzer_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))
...
#define rcv_bits                		pr_state_ptr->rcv_bits
#define statistic               		pr_state_ptr->statistic
#define max_delay               		pr_state_ptr->max_delay
#define mean_delay              		pr_state_ptr->mean_delay
#define Umax                    		pr_state_ptr->Umax
#define nbr_pkt_rcv             		pr_state_ptr->nbr_pkt_rcv
#define log                     		pr_state_ptr->log
#define enable_log              		pr_state_ptr->enable_log
#define beacons_bits            		pr_state_ptr->beacons_bits
#define nbr_beacons             		pr_state_ptr->nbr_beacons
#define my_attributes           		pr_state_ptr->my_attributes
#define acks_bits               		pr_state_ptr->acks_bits
#define nbr_acks                		pr_state_ptr->nbr_acks
...
#define FIN_PREAMBLE_DEC	wpan_analyzer_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((wpan_analyzer_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void wpan_analyzer (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_wpan_analyzer_init (int * init_block_ptr);
	void _op_wpan_analyzer_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_wpan_analyzer_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_wpan_analyzer_alloc (VosT_Obtype, int);
	void _op_wpan_analyzer_svar (void *, const char *, void **);
...
void
wpan_analyzer (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (wpan_analyzer ());

		{


		FSM_ENTER_NO_VARS ("wpan_analyzer")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "wpan_analyzer [init enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_analyzer [init enter execs]", state0_enter_exec)
				{
				wpan_analyzer_init ();
			...
			FSM_STATE_EXIT_FORCED (0, "init", "wpan_analyzer [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_12", "wpan_analyzer [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "wpan_analyzer [idle enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_analyzer [idle enter execs]", state1_enter_exec)
				{
			...
			FSM_EXIT (3,"wpan_analyzer")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "wpan_analyzer [idle exit execs]")


			/** state (idle) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_6", "wpan_analyzer [idle -> idle : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_wpan_analyzer_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_wpan_analyzer_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_wpan_analyzer_terminate ())
...
/* local variable prs_ptr in _op_wpan_analyzer_svar function. */
...
VosT_Obtype
_op_wpan_analyzer_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_wpan_analyzer_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (wpan_analyzer)",
		sizeof (wpan_analyzer_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_wpan_analyzer_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	wpan_analyzer_state * ptr;
	FIN_MT (_op_wpan_analyzer_alloc (obtype))

	ptr = (wpan_analyzer_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "wpan_analyzer [init enter execs]";
#endif
		}
...
void
_op_wpan_analyzer_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	wpan_analyzer_state		*prs_ptr;

	FIN_MT (_op_wpan_analyzer_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (wpan_analyzer_state *)gen_ptr;
