/* Process model C++ form file: wpan_appl_mod_threshold.pr.cpp */
/* Portions of this file copyright 2014 by Riverbed Technology. All rights reserved. */


/*
 =========================== NOTE ==========================
 This file is automatically generated from wpan_appl_mod_threshold.pr.m
...
const char wpan_appl_mod_threshold_pr_cpp [] = "MIL_3_Tfile_Hdr_ 1800 30A modeler 7 5511DFB7 5511DFB7 1 rhel04 chaganti 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 468d 2                                                                                                                                                                                                                                                                                                                                                                                                          ";
#include <string.h>
...
int bloodPressure[20] = {183,180,180,187,156,180,194,210,180,180,175,172,180,175,200,180,180,180,180,180};
int heartRate[20] = {60,70,80,80,90,60,70,60,60,70,120,120,60,70,70,70,70,70,70,120};
int spo2[10] = {50,120,60,130,50,50,50,50,50,120};
int bloodSugar[10] = {110,110,110,140,140,150,150,140,150,150};
int temperature[10] = {37,37,37,37,38,38,38,38,38,38};
int value1 = 0;
int value2 = 0;
int value3 = 0;
int value4 = 0;
...
class wpan_appl_mod_threshold_state
	{
	private:
	...
	public:
		wpan_appl_mod_threshold_state (void);
		...
		int	                    		pay_load                                        ;
		int	                    		sizeArray                                       ;
		Stathandle	             		pkts_sent_node_stathandle                       ;

		/* FSM code */
		void wpan_appl_mod_threshold (OP_SIM_CONTEXT_ARG_OPT);
		/* Diagnostic Block */
		void _op_wpan_appl_mod_threshold_diag (OP_SIM_CONTEXT_ARG_OPT);
...
VosT_Obtype wpan_appl_mod_threshold_state::obtype = (VosT_Obtype)OPC_NIL;
...
#define pkts_sent_node_stathandle		op_sv_ptr->pkts_sent_node_stathandle
...
#define FIN_PREAMBLE_DEC	wpan_appl_mod_threshold_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((wpan_appl_mod_threshold_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
static void wpan_appl_init() {
	...
	pkts_sent_node_stathandle		= op_stat_reg ("Application.Pay load value sent from a node",	OPC_STAT_INDEX_NONE, OPC_STAT_LOCAL);
...
static void wpan_be_packet_generate() {
	...
	if (nsdu_size < 0)
		...
		printf("NSDU size before payload : %d\n", nsdu_size);
	...
	/*if (prg_string_order_case_insensitive(device_name,"Blood Pressure") == 0){
		...
		next_intarr_time = floor(op_dist_uniform (2));
		wpan_process_payload();
		printf("Next Arrival time : %f\n", next_intarr_time);
		}
		...
		next_intarr_time = floor(op_dist_uniform (30));
		wpan_process_payload();
		printf("Next Arrival time : %f\n", next_intarr_time);
		}
	...
	if(OUT_OF_NORMAL_DATA_BP | OUT_OF_NORMAL_DATA_HR | OUT_OF_NORMAL_DATA_SPO2 | OUT_OF_NORMAL_DATA_Temp | OUT_OF_NORMAL_DATA_BS){
		...
		for(Index = 0 ; Index < 6 ; Index ++){
			...
			if (prg_string_order_case_insensitive(device_name, fuzzyCheckPtr[Index].nodeName) == 0){
					op_stat_write(pkts_sent_node_stathandle,fuzzyCheckPtr[Index].payLoad);
...
//	fuzzyGroup[Index].nodeName = (char *)op_prg_mem_alloc(sizeof(char));
	...
	if (prg_string_order_case_insensitive(device_name,"Blood Pressure") == 0){
		fuzzyGroup[0].payLoad = bloodPressure[value1];
		value1++;
		...
		fuzzyGroup[1].payLoad = heartRate[value2];
		...
		fuzzyGroup[2].payLoad = spo2[value3];
		value3++;
		...
		fuzzyGroup[3].payLoad = temperature[value4];
		value4++;
		...
		fuzzyGroup[4].payLoad = bloodSugar[value5];
...
#undef pay_load
#undef sizeArray
#undef pkts_sent_node_stathandle

/* Access from C kernel using C linkage */
extern "C"
{
	VosT_Obtype _op_wpan_appl_mod_threshold_init (int * init_block_ptr);
	VosT_Address _op_wpan_appl_mod_threshold_alloc (VosT_Obtype, int);
	void wpan_appl_mod_threshold (OP_SIM_CONTEXT_ARG_OPT)
		{
		((wpan_appl_mod_threshold_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->wpan_appl_mod_threshold (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_wpan_appl_mod_threshold_svar (void *, const char *, void **);

	void _op_wpan_appl_mod_threshold_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((wpan_appl_mod_threshold_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_wpan_appl_mod_threshold_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_wpan_appl_mod_threshold_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (wpan_appl_mod_threshold_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
wpan_appl_mod_threshold_state::wpan_appl_mod_threshold (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (wpan_appl_mod_threshold_state::wpan_appl_mod_threshold ());
	try
		{


		FSM_ENTER ("wpan_appl_mod_threshold")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "wpan_appl_mod_threshold [init enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_appl_mod_threshold [init enter execs]", state0_enter_exec)
				{
				/* Initialization */
				wpan_appl_init ();
				}
			...
			FSM_STATE_EXIT_FORCED (0, "init", "wpan_appl_mod_threshold [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_1", "wpan_appl_mod_threshold [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "wpan_appl_mod_threshold [idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"wpan_appl_mod_threshold")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "wpan_appl_mod_threshold [idle exit execs]")
				FSM_PROFILE_SECTION_IN ("wpan_appl_mod_threshold [idle exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("wpan_appl_mod_threshold [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "SEND_BE", "", "idle", "send BE", "send best-effort packet", "wpan_appl_mod_threshold [idle -> send BE : SEND_BE / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "SEND_RT", "", "idle", "send RT", "send real-time packet", "wpan_appl_mod_threshold [idle -> send RT : SEND_RT / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "RECEIVE_PCK", "", "idle", "receive pck", "receive packet from NWK", "wpan_appl_mod_threshold [idle -> receive pck : RECEIVE_PCK / ]")
				FSM_CASE_TRANSIT (3, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_13", "wpan_appl_mod_threshold [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "send BE", state2_enter_exec, "wpan_appl_mod_threshold [send BE enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_appl_mod_threshold [send BE enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "send BE", "wpan_appl_mod_threshold [send BE exit execs]")


			/** state (send BE) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send BE", "idle", "tr_6", "wpan_appl_mod_threshold [send BE -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send RT) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send RT", state3_enter_exec, "wpan_appl_mod_threshold [send RT enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_appl_mod_threshold [send RT enter execs]", state3_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "send RT", "wpan_appl_mod_threshold [send RT exit execs]")


			/** state (send RT) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send RT", "idle", "tr_8", "wpan_appl_mod_threshold [send RT -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (receive pck) enter executives **/
			FSM_STATE_ENTER_FORCED (4, "receive pck", state4_enter_exec, "wpan_appl_mod_threshold [receive pck enter execs]")
				FSM_PROFILE_SECTION_IN ("wpan_appl_mod_threshold [receive pck enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (4, "receive pck", "wpan_appl_mod_threshold [receive pck exit execs]")


			/** state (receive pck) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "receive pck", "idle", "tr_10", "wpan_appl_mod_threshold [receive pck -> idle : default / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"wpan_appl_mod_threshold")
		}
	catch (...)
		{
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (wpan_appl_mod_threshold)",
...
void
wpan_appl_mod_threshold_state::_op_wpan_appl_mod_threshold_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}

void
wpan_appl_mod_threshold_state::operator delete (void* ptr)
	{
	FIN (wpan_appl_mod_threshold_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

wpan_appl_mod_threshold_state::~wpan_appl_mod_threshold_state (void)
	{

	FIN (wpan_appl_mod_threshold_state::~wpan_appl_mod_threshold_state ())
...
void *
wpan_appl_mod_threshold_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (wpan_appl_mod_threshold_state::operator new ());

	new_ptr = Vos_Alloc_Object (wpan_appl_mod_threshold_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
wpan_appl_mod_threshold_state::wpan_appl_mod_threshold_state (void) :
		_op_current_block (0)
	{
		...
		_op_current_state = "wpan_appl_mod_threshold [init enter execs]";
#endif
	}

VosT_Obtype
_op_wpan_appl_mod_threshold_init (int * init_block_ptr)
	{
	FIN_MT (_op_wpan_appl_mod_threshold_init (init_block_ptr))

	wpan_appl_mod_threshold_state::obtype = Vos_Define_Object_Prstate ("proc state vars (wpan_appl_mod_threshold)",
		sizeof (wpan_appl_mod_threshold_state));
	*init_block_ptr = 0;

	FRET (wpan_appl_mod_threshold_state::obtype)
	}

VosT_Address
_op_wpan_appl_mod_threshold_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	wpan_appl_mod_threshold_state * ptr;
	FIN_MT (_op_wpan_appl_mod_threshold_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new wpan_appl_mod_threshold_state;
...
void
_op_wpan_appl_mod_threshold_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	wpan_appl_mod_threshold_state		*prs_ptr;

	FIN_MT (_op_wpan_appl_mod_threshold_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (wpan_appl_mod_threshold_state *)gen_ptr;
	...
	if (strcmp ("pkts_sent_node_stathandle" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->pkts_sent_node_stathandle);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
