/* Process model C++ form file: wpan_application_router.pr.cpp */
...
const char wpan_application_router_pr_cpp [] = "MIL_3_Tfile_Hdr_ 175A 30A op_runsim_opt 7 54C6C00C 54C6C00C 1 e202f01 chaganti 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 3282 5                                                                                                                                                                                                                                                                                                                                                                                                   ";
#include <string.h>
...
class wpan_application_router_state
	{
	private:
	...
	public:
		wpan_application_router_state (void);
...
VosT_Obtype wpan_application_router_state::obtype = (VosT_Obtype)OPC_NIL;
...
extern "C"
{
	...
	void wpan_application_router (OP_SIM_CONTEXT_ARG_OPT)
		{
		((wpan_application_router_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->wpan_application_router (OP_SIM_CONTEXT_PTR_OPT);
		}
	...
	void _op_wpan_application_router_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((wpan_application_router_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_wpan_application_router_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_wpan_application_router_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (wpan_application_router_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
wpan_application_router_state::wpan_application_router (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (wpan_application_router_state::wpan_application_router ());
	try
		{
		...
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (wpan_application_router)",
...
void
wpan_application_router_state::_op_wpan_application_router_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}

void
wpan_application_router_state::operator delete (void* ptr)
	{
	FIN (wpan_application_router_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

wpan_application_router_state::~wpan_application_router_state (void)
	{

	FIN (wpan_application_router_state::~wpan_application_router_state ())
...
void *
wpan_application_router_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (wpan_application_router_state::operator new ());

	new_ptr = Vos_Alloc_Object (wpan_application_router_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
wpan_application_router_state::wpan_application_router_state (void) :
		_op_current_block (0)
	{
		...
		_op_current_state = "wpan_application_router [init enter execs]";
#endif
	}
...
_op_wpan_application_router_init (int * init_block_ptr)
	...
	wpan_application_router_state::obtype = Vos_Define_Object_Prstate ("proc state vars (wpan_application_router)",
		sizeof (wpan_application_router_state));
	*init_block_ptr = 0;

	FRET (wpan_application_router_state::obtype)
	}

VosT_Address
_op_wpan_application_router_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	wpan_application_router_state * ptr;
	FIN_MT (_op_wpan_application_router_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new wpan_application_router_state;
